    ...
    body: async function body(req, res, event) {
        ...
        try { //path valid?
            ...
            if (!stats.isFile()) { //request isDir
                ...
                idx = files.find((file) => event.server.opts.index.test(file)); //finds index -> blocks indexing
                if (idx) { //has index file
                    ...
                    try { //has .noindex?
                        ...
                        if (noidx) { //.noindex filled
                            ...
                            res.end(await index(event, files.filter((file) => regs.some((reg) => !reg.test(file))), uri));
                        }
                        else { //.noindex empty
...
async function index(event, files, uri) {
    ...
    let pth = uri.pathname.replace(new RegExp("^." + event.server.opts.root, "i"), '');
    try {
